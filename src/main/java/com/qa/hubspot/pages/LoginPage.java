...
public class LoginPage extends BasePage {
	...
	public String getLoginPageTitle() {
		return elementUtil.waitForTitleToBePresent(Constants.LOGIN_PAGE_TITLE, dur);
	}

	@Step("check sign up link on login page....")
	public boolean verifySignUpLink() {
		elementUtil.waitForElementPresent(signUpLink, dur);
		return elementUtil.doIsDisplayed(signUpLink);
	}

	@Step("login to app with username: {0} and password: {1}")
	public HomePage doLogin(String username, String password) {
		elementUtil.waitForElementPresent(this.username, dur);
